...
class AugmentPipe:
    ...
    def __call__(self, images):
        ...
        if self.scale > 0:
            ...
            s = torch.exp(w.mul(self.scale_std) * math.log(2))
        ...
        if self.aniso > 0:
            ...
            s = torch.exp(w.mul(self.aniso_std) * math.log(2))
        ...
        if self.contrast > 0:
            ...
            c = torch.exp(w.mul(self.contrast_std) * math.log(2))
        ...
        if self.saturation > 0:
            ...
            saturation_factor = torch.exp(w.mul(self.saturation_std) * math.log(2))
            M = (luma_axis.ger(luma_axis) + (I_4 - luma_axis.ger(luma_axis)) * saturation_factor) @ M
            labels += [w]
