...
def main(input_dir, output_dir, features_path, data, max_size, cache, workers, batch, batch_gpu, seed, normalize):
    ...
    is_file_flg = is_file(features_path)
    if not is_file_flg:
        ...
        with torch.no_grad():
            for dataset_iter in tqdm(dataset_iterator):
                ...
                images_normalized = images.float() / 127.5 - 1
                local_features = feature_extractor((pad_image(images_normalized) + 1) / 2. ).cpu()
    ...
    c = dnnlib.EasyDict()
    c.dataset_kwargs = dnnlib.EasyDict(class_name='training.dataset.ImageFolderDataset', path=input_dir, use_labels=False, xflip=False, cache=cache, max_size=max_size)
    ...
    with torch.no_grad():
        for iter_index, dataset_iter in enumerate(tqdm(outputs_dataset_obj)):
            ...
            save_image((images[0] / 127.5) - 1, os.path.join(output_dir, f'{iter_index}_generated_image_{iter_index}.png'))
            ...
            for k in range(1):
                ...
                save_image((images / 127.5) - 1, os.path.join(output_dir, f'{iter_index}_nearest_neighbors_{k}_dataset_index_{i}.png'))
    ...
    sns.distplot(np.array(max_products))
    plt.savefig(output_dir + "/" + "products-hist")
